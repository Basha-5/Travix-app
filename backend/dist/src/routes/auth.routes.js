...
router.post('/request-email-otp', async (req, res, next) => {
    try {
        ...
        // Send token strictly to user's real email via Gmail SMTP
        const template = email_service_1.emailTemplates.otpCode(otp);
        await (0, email_service_1.sendEmail)({ to: email, ...template });
        res.json({
            success: true,
            message: `Authentication token sent to ${email}. Please check your email inbox on your mobile/desktop.`,
        });
    }
    catch (error) {
        next(error);
